...
from characters import spawnBarbarian, move_barbarians, spawnArcher, move_archers,spawnSArcher,move_Sarchers, spawnDragon, spawnHealer, move_healers,move_dragons, spawnBalloon, move_balloons, clearTroops
...
V = village.createVillage(level,King)
os.system('clear')
printMap(V)
...
while(True):
    ...
    if ch == 'w':
        ...
        spawnSArcher(V.spawn_points[0])
    elif ch == 'u':
        spawnSArcher(V.spawn_points[1])
    elif ch == 'y':
        spawnSArcher(V.spawn_points[2])
    elif ch == 'f':
