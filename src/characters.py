...
class StealthArcher(Archer):
    ...
    def deal_damage(self, enemy):
        if not self.invisible:
            super().deal_damage(enemy)

    def kill(self):
        self.alive = False
        sArchers.remove(self)
...
class Healer:
    ...
    def findClosest(self, King):
        nearest_troop = None
        nearest_dist = float("inf")
        troops = barbarians + archers + sArchers + dragons + balloons + [King]
        for troop in troops:
            if troop != self and troop.alive and troop.max_health - troop.health >= 20 :
    ...
    def move(self, King):
        ...
        nearest_troop = self.findClosest(King)
        if nearest_troop is None:
            return 
        r = abs(nearest_troop.position[0] - self.position[0])
        c = abs(nearest_troop.position[1] - self.position[1])
        if r + c <= 7:
            self.heal_troop(nearest_troop)
            troops  = barbarians+archers+sArchers+dragons+balloons+[King]
            for other_troop in troops:
                if other_troop != nearest_troop:
                    distance_to_other_troop = abs(other_troop.position[0] - nearest_troop.position[0]) + abs(other_troop.position[1] - nearest_troop.position[1])
                    if distance_to_other_troop <= 1 and other_troop.health < other_troop.max_health and other_troop.max_health - other_troop.health >= 20:
                        self.heal_troop(other_troop)
            return
        elif r == 0:
            if nearest_troop.position[1] > self.position[1]:
                for i in range(self.speed):
                    r = self.position[0]
                    c = self.position[1] + 1
                    self.position[1] += 1
                    if abs(nearest_troop.position[1] - self.position[1]) == 1:
                        break
            else:
                for i in range(self.speed):
                    r = self.position[0]
            ...
            if nearest_troop.position[0] > self.position[0]:
                for i in range(self.speed):
                    r = self.position[0] + 1
                    c = self.position[1]
                    self.position[0] += 1
                    if self.position[0] == nearest_troop.position[0]:
                        return
            else:
                for i in range(self.speed):
                    r = self.position[0] - 1
            ...
            if nearest_troop.position[1] > self.position[1]:
                for i in range(self.speed):
                    ...
                    c = self.position[1] + 1
                    self.position[1] += 1
                    if self.position[1] == nearest_troop.position[1]:
                        return
            else:
                for i in range(self.speed):
                    r = self.position[0]
...
def spawnSArcher(pos):
    if pt.troop_limit["sArcher"] <= troops_spawned["sArcher"]:
        return

    # convert tuple to list
    pos = list(pos)
    sArcher = StealthArcher(pos)
    troops_spawned["sArcher"] += 1
    sArchers.append(sArcher)
...
def move_Sarchers(V, type):
    for sarcher in sArchers:
        sarcher.update()
    if type == 1:
        for sArcher in sArchers:
