...
def SarchColor(sArch):
    cantGetAttacked = sArch.invisible
    if(sArch.invisible):
        return Back.BLUE
    else:
        return Back.YELLOW
...
def printMap(V):
    ...
    for sArch in sArchers:
        a= 2*sArch.position[0]
        b = 2*sArch.position[1]
        map_matrix[a][b] = SarchColor(sArch) + Fore.BLACK + 'ST'
        map_matrix[a+1][b] = SarchColor(sArch) + Fore.BLACK + 'AR'
    ...
    for healer in healers:
        ...
        map_matrix[a+1][b] = healerColor(healer) + Fore.BLACK + 'ER'
        map_matrix[a][b+1] = healerColor(healer) + Fore.BLACK + 'AL'
        map_matrix[a+1][b+1] = healerColor(healer) + Fore.BLACK + '  '
