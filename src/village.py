...
import collections
from characters import barbarians, dragons,balloons, archers, sArchers


class Village:
    def __init__(self, config, level,King):
        self.level = level
        self.king = King
        self.hut_levels = config['hutLevels']
        self.can_levels = config['canLevels']
        self.wiz_levels = config['wizLevels']
        self.wall_level = config['wallLevel']
    ...
    def generate_map(self):
        ...
        for hut in self.huts:
            h=h+1                                           # hut is 2x2
            hut_obj = bd.Hut(hut, self,self.hut_levels[h])
        ...
        for wall in self.walls_top:                                     # wall is 1x1
            wall_obj = bd.Wall(wall, self,self.wall_level)
        ...
        for cannon in self.cannons:                                     # cannon is 2x2
            c=c+1
            cannon_obj = bd.Cannon(cannon, self,self.can_levels[c])
        ...
        for wizard_tower in self.wizard_towers:                          # wizard tower is 1X1
            w=w+1
            wizard_tower_obj = bd.WizardTower(wizard_tower, self,self.wiz_levels[w])
    ...
    def check_if_game_over(self, King):
        ...
        if len(all_buildings) == 0 and self.town_hall_obj == None:
            if self.level != 3:
                ...
                return 2
        elif King.alive == False and len(barbarians) == 0 and len(sArchers) == 0 and len(dragons) == 0 and len(balloons) == 0 and len(archers) == 0:
            return 3
        else:
            return 0


def createVillage(level,King):
    village = Village(getConfig(level), level, King)
    return village
